...
def adjust_arguments(args: dict[str, Any]) -> None:
	if any(part.fs_type==disk.FilesystemType('btrfs') for part in args['disk_config'].device_modifications[0].partitions):
		args['packages_aur'].append("btrfs-assistant")
		print('\nHELLO\n')
	
	if args.get("cyber", False):
		args["packages"].append("wireshark-qt", "python-pwntools", "docker", "gdb", "python-z3-solver", "ltrace", "strace", "python-pycryptodome", "nmap", "elfutils", "patchelf", "john", "binwalk")
		args["packages_aur"].append("burpsuite", "ghidra-git", "stegsolve")
		args["custom-commands"].append("usermod -aG wireshark,docker archi")
		args["services"].append("docker.socket")
